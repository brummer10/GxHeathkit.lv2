...
bool gx_gui_open_display(gx_HeathkitUI *ui) {
	return true; // STUB
}

void gx_gui_create_window_and_surface(gx_HeathkitUI *ui) {
	// prepare window class
	static TCHAR szClassName[] = TEXT("gx_HeathkitUIClass");
	WNDCLASS wndclass = {0};
...
void gx_gui_register_controller_message(gx_HeathkitUI *ui) {
	// dummy, not required on MSWin
}

void gx_gui_destroy_main_window(gx_HeathkitUI *ui) {
	DestroyWindow(ui->win);
	// safe to use: doesnt unregister if there are still windows of this class
	UnregisterClass(TEXT("gx_HeathkitUIClass"), NULL);
}

void gx_gui_resize_surface(gx_HeathkitUI *ui) {
	RECT rect;
...
void gx_gui_send_controller_event(gx_HeathkitUI *ui, int controller) {
...
void event_handler(gx_HeathkitUI *ui) {
}
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	gx_HeathkitUI *ui = (gx_HeathkitUI *)GetWindowLongPtr(hwnd, GWLP_USERDATA);
