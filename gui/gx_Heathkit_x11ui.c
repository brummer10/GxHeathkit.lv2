...
static void resize_event(gx_HeathkitUI *ui);
...
cairo_surface_t *cairo_image_surface_create_from_stream (gx_HeathkitUI* ui, const unsigned char* name) {
...
static LV2UI_Handle instantiate(const LV2UI_Descriptor * descriptor,
	...
	gx_HeathkitUI* ui = (gx_HeathkitUI*)malloc(sizeof(gx_HeathkitUI));
	...
	ui->controls[0] = (gx_controller) {{ 1.0, 1.0, 0.0, 1.0, 1.0}, { 30, 30, 61, 61}, false,"POWER", BSWITCH, BYPASS};
	ui->controls[1] = (gx_controller) {{ 0.5, 0.5, 0.0, 1.0, 0.01},{ 120, 30, 61, 61}, false,"LEVEL", KNOB, LEVEL};
	ui->controls[2] = (gx_controller) {{ 0.5, 0.5, 0.0, 1.0, 0.01},{ 210, 30, 61, 61}, false,"TONE", KNOB, TONE};
	ui->block_event = -1;
...
static void cleanup(LV2UI_Handle handle) {
	gx_HeathkitUI* ui = (gx_HeathkitUI*)handle;
	cairo_destroy(ui->cr);
...
static void knob_expose(gx_HeathkitUI *ui,gx_controller* knob) {
...
static void bypass_expose(gx_HeathkitUI *ui, gx_controller* switch_) {
	cairo_set_operator(ui->crf,CAIRO_OPERATOR_CLEAR);
	cairo_paint(ui->crf);
	cairo_set_operator(ui->crf,CAIRO_OPERATOR_OVER);
...
static void draw_controller(gx_HeathkitUI *ui, gx_controller* controller) {
...
static void _expose(gx_HeathkitUI *ui) {
	static const char* plug_name = "GxHeathkit" ;
...
static void controller_expose(gx_HeathkitUI *ui, gx_controller * control) {
	cairo_push_group (ui->cr);
	cairo_scale (ui->cr, ui->rescale.x, ui->rescale.y);
...
static void send_controller_event(gx_HeathkitUI *ui, int controller) {
...
static void check_value_changed(gx_HeathkitUI *ui, int i, float* value) {
...
static void check_is_active(gx_HeathkitUI *ui, int i, bool set) {
...
static bool aligned(int x, int y, gx_controller *control, gx_HeathkitUI *ui) {
...
static bool get_active_ctl_num(gx_HeathkitUI *ui, int *num) {
	static bool ret;
	ret = false;
	for (int i=0;i<CONTROLS;i++) {
...
static bool get_active_controller_num(gx_HeathkitUI *ui, int *num) {
	for (int i=0;i<CONTROLS;i++) {
...
static void scroll_event(gx_HeathkitUI *ui, int direction) {
	float value;
	int num;
...
static void switch_event(gx_HeathkitUI *ui, int i) {
...
static void button1_event(gx_HeathkitUI *ui, double* start_value) {
	int num;
	if (get_active_ctl_num(ui, &num)) {
...
static void motion_event(gx_HeathkitUI *ui, double start_value, int m_y) {
...
static void set_key_value(gx_HeathkitUI *ui, int set_value) {
	float value = 0.0;
	int num;
	if (get_active_controller_num(ui, &num)) {
...
static void key_event(gx_HeathkitUI *ui, int direction) {
	float value;
	int num;
	if (get_active_controller_num(ui, &num)) {
		value = min(ui->controls[num].adj.max_value,max(ui->controls[num].adj.min_value, 
...
static void set_previous_controller_active(gx_HeathkitUI *ui) {
	int num;
	if (get_active_controller_num(ui, &num)) {
...
static void set_next_controller_active(gx_HeathkitUI *ui) {
	int num;
	if (get_active_controller_num(ui, &num)) {
		ui->controls[num].is_active = false;
		send_controller_event(ui, num);
...
void get_last_active_controller(gx_HeathkitUI *ui, bool set) {
...
static void event_handler(gx_HeathkitUI *ui) {
	XEvent xev;
